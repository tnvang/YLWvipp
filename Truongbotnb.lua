...
local function isVisible(targetPart, character)
    local castParts = camera:GetPartsObscuredByLetters({targetPart.Position}, {camera, LocalPlayer.Character, character})
    return #castParts == 0
end
...
-- // VÒNG LẶP HỆ THỐNG XỬ LÝ CHÍNH //
RunService.RenderStepped:Connect(function()
    updateFOV()
    
    -- Xử lý Aimbot & Tìm mục tiêu gần nhất
        ...
        for _, v in ipairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
                ...
                if onScreen then
                    ...
                    if mouseDist < Config.FOVRadius and mouseDist < shortestDistance then
                        local distanceToPlayer = (camera.CFrame.Position - hrp.Position).Magnitude
                        if distanceToPlayer <= Config.Distance and isVisible(hrp, v.Character) then
    ...
    -- Xử lý ESP Tên và ESP Box
    for _, oct in pairs(SystemState.ESPOcts) do
        if oct.Box then oct.Box.Visible = false end
        if oct.Text then oct.Text.Visible = false end
    end
    ...
    for _, v in ipairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
            ...
            if onScreen then
                if not SystemState.ESPOcts[v] then
                    SystemState.ESPOcts[v] = {
                        Box = Drawing.new("Square"),
                        Text = Drawing.new("Text")
                    }
                end
                
                local drawObjects = SystemState.ESPOcts[v]
                
                if Config.ESPBoxEnabled then
                    local sizeX = 2000 / screenPos.Z
                    local sizeY = 3000 / screenPos.Z
                    drawObjects.Box.Size = Vector2.new(sizeX, sizeY)
                    drawObjects.Box.Position = Vector2.new(screenPos.X - sizeX / 2, screenPos.Y - sizeY / 2)
                    drawObjects.Box.Color = Color3.fromRGB(255, 0, 0)
                    drawObjects.Box.Thickness = 1.5
                    drawObjects.Box.Filled = false
                    drawObjects.Box.Visible = true
                end
                
                if Config.ESPNameEnabled then
                    drawObjects.Text.Text = v.Name .. " [" .. math.round((camera.CFrame.Position - hrp.Position).Magnitude) .. "m]"
                    drawObjects.Text.Size = math.clamp(16 - (screenPos.Z / 20), 10, 16)
                    drawObjects.Text.Position = Vector2.new(screenPos.X, screenPos.Y - (3500 / screenPos.Z) / 2 - 15)
                    drawObjects.Text.Color = Color3.fromRGB(255, 255, 255)
                    drawObjects.Text.Center = true
                    drawObjects.Text.Outline = true
                    drawObjects.Text.Visible = true
                end
            end
        end
    end
...
-- Gỡ ESP khi người chơi out server
Players.PlayerRemoving:Connect(function(player)
    if SystemState.ESPOcts[player] then
        if SystemState.ESPOcts[player].Box then SystemState.ESPOcts[player].Box:Remove() end
        if SystemState.ESPOcts[player].Text then SystemState.ESPOcts[player].Text:Remove() end
        SystemState.ESPOcts[player] = nil
    end
end)

-- // XỬ LÝ NÚT CHECK KEY (NÚT XÁC NHẬN) //
...
WrongBtn.MouseButton1Click:Connect(function()
    -- Nếu chọn sai thì tự động văng game (Kick)

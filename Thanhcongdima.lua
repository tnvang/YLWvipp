...
local State = {
    ...
    Aim_Enabled = false,
    Aim_Radius = 150,
    Aim_Mode = "All" -- Chế độ: "All", "Players", "NPCs"
}

-- Quản lý kết nối sự kiện hệ thống để tránh lỗi bộ nhớ
local Connections = {}
...
-- Tạo vòng tròn FOV Aimbot nguyên bản bằng thư viện Drawing
...
-- =================================================================
-- 3. KHỞI TẠO GIAO DIỆN CHÍNH & HIỆU ỨNG VIỀN ĐỔI MÀU (RAINBOW BORDER)
-- =================================================================
local gui = Instance.new("ScreenGui")
gui.Name = "ThanhCongHub_v2.1.0"
...
-- Dòng hiển thị chỉ số phần cứng thời gian thực (FPS/Ping/RAM)
local HardwareStatsLabel = Instance.new("TextLabel")
HardwareStatsLabel.Size = UDim2.new(0.5, -10, 0, 40)
HardwareStatsLabel.Position = UDim2.new(0.5, 0, 0, 0)
HardwareStatsLabel.BackgroundTransparency = 1
HardwareStatsLabel.Text = "FPS: -- | Ping: --ms | RAM: --MB"
HardwareStatsLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
HardwareStatsLabel.Font = Enum.Font.SourceSansBold
HardwareStatsLabel.TextSize = 12
HardwareStatsLabel.TextXAlignment = Enum.TextXAlignment.Right
HardwareStatsLabel.Parent = Main

-- Các nút thao tác đóng/mở cửa sổ
...
-- Nút nổi khi tắt Menu để mở lại
...
-- Hàm tạo đường viền Rainbow bo góc thông minh chạy nền liên tục
...
-- Kích hoạt viền chuyển màu Rainbow cho toàn bộ khung chính và nút thao tác
...
-- =================================================================
-- Vùng chứa cuộn danh sách chức năng bên trái
...
-- Bảng hiển thị thông tin Server bên phải
...
-- =================================================================
-- 5. LOGIC DI CHUYỂN KÉO THẢ (DRAG) & THU NHỎ/ĐÓNG/MỞ CỦA SỔ CHUẨN
-- =================================================================
...
-- Kiểm tra đường thẳng tầm nhìn giữa Camera và mục tiêu (Không bị tường che)
...
-- Tìm mục tiêu gần tâm hồng tâm màn hình nhất dựa trên cài đặt chế độ quét
...
-- Bật/tắt Aimbot và vòng tròn khóa mục tiêu FOV
...
-- Chức năng bay cao cấp dành cho Mobile (Tự điều hướng theo góc nhìn Camera)
...
-- Khởi tạo các hiệu ứng bước đi đặc biệt (Đuôi cầu vồng, vệt tuyết rơi)
...
-- Chức năng tách rời Camera để bay tự do (Không di chuyển nhân vật)
...
-- Tạo cấu trúc vẽ khung định vị người chơi (ESP Engine)
...
Players.PlayerRemoving:Connect(function(p)
    if State.ESP_Storage[p] then
        State.ESP_Storage[p].Box:Destroy()
        State.ESP_Storage[p].Text:Destroy()
...
local TargetModeBtn = createButton("🎯 MỤC TIÊU: TẤT CẢ (NPC + PLAYER)", 2, function()
    if State.Aim_Mode == "All" then State.Aim_Mode = "Players" TargetModeBtn.Text = "🎯 MỤC TIÊU: CHỈ NGƯỜI CHƠI"
    elseif State.Aim_Mode == "Players" then State.Aim_Mode = "NPCs" TargetModeBtn.Text = "🎯 MỤC TIÊU: CHỈ NPC / ZOMBIE"
    else State.Aim_Mode = "All" TargetModeBtn.Text = "🎯 MỤC TIÊU: TẤT CẢ (NPC + PLAYER)" end

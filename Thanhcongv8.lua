-- [[ PREMIUM MOBILE HUB V4.6 - ULTIMATE MOBILE FIX ]] --
...
if _G.MobileHubExecutedFinalV4_6 then
    print("[Mobile Hub] Hub V4.6 đang hoạt động!")
    return
else
    _G.MobileHubExecutedFinalV4_6 = true
end
...
ScreenGui.Name = "PremiumMobileHub_V4_6"
...
Title.Text = "PREMIUM MOBILE HUB V4.6" Title.TextColor3 = Color3.fromRGB(255, 255, 255)
...
-- [FIX COMPLETE] CHỨC NĂNG LEO TƯỜNG KHÔNG BỊ TUỘT
function toggleWallWalk(state)
    ...
    if state then
        ...
        WallConnection = RunService.Heartbeat:Connect(function()
            ...
            -- Quét tia ngang để tìm bề mặt tường xung quanh nhân vật
            ...
            if result and math.abs(result.Normal.Y) < 0.85 then
                ...
                -- Khóa lực trọng lực rơi tự do, ép đứng yên và di chuyển tuyến tính
                root.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                
                -- Tính toán hướng xoay áp sát mặt phẳng tường mượt mà
                ...
                -- Cho phép Joystick di chuyển đi lên/xuống/ngang trên mặt tường chính xác
...
-- [FIX COMPLETE] TRIỆU HỒI BOT ĐỔI MÀU LƠ LỬNG SAU LƯNG VUI MẮT
function toggleFollowBot(state)
    ...
    if state then
        ...
        -- Tạo mô hình Khối cầu phát sáng Neon đại diện cho Bot
        ...
        -- Đính kèm thêm 2 cánh nhỏ hai bên cho sinh động
        ...
        FollowConnection = RunService.RenderStepped:Connect(function()
            ...
            -- Đổi màu RGB liên tục chạy mượt cho Bot phát sáng
            hue = (hue + 0.005) % 1
            bot.Color = Color3.fromHSV(hue, 1, 1)
            
            -- Tính toán vị trí lơ lửng phía sau lưng nhân vật (Cách 3.5 Block về sau, 2 Block lên trên)
                     local backOffset = root.CFrame * CFrame.new(0, 2.2, 3.5)
            
            -- Hiệu ứng bay bập bênh lên xuống (Wave)
            local bobbing = math.sin(tick() * 4) * 0.4
            local finalTargetPos = backOffset.Position + Vector3.new(0, bobbing, 0)
            
            -- Cập nhật CFrame mượt
...
-- [FIX COMPLETE] BAY THEO CHÍNH XÁC GÓC NHÌN CAMERA MOBILE
    ...
    if enable then
        ...
        flyRenderConnection = RunService.Heartbeat:Connect(function()
            ...
            if moveDir.Magnitude > 0 then
                -- Đồng bộ hóa Vector di chuyển dựa hoàn toàn theo hướng trục dọc xiên của Camera
                local rightVec = Camera.CFrame.RightVector
                local forwardVec = camLook
                
                -- Kết hợp Joystick để bay đúng hướng mặt đang nhìn nhìn thẳng
...
-- [FIX COMPLETE] FREECAM KHÓA NHÂN VẬT AN TOÀN, VUỐT ĐỂ XOAY CAM
function toggleFreecam(state)
    ...
    if state then
        if root then 
            ...
            root.Anchored = true -- Khóa nhân vật đứng im tại chỗ an toàn hoàn toàn
        end
        ...
        -- Lắng nghe thao tác chạm vuốt màn hình để xoay camera tự do trên Mobile
        ...
        FreecamConnection = RunService.RenderStepped:Connect(function()
            ...
            -- Dùng Joystick di chuyển camera trong không gian tự do
...
-- [FIX COMPLETE] HIỆU ỨNG THIẾT KẾ LẠI HOÀN TOÀN BẰNG PARTICLE GỐC KHÔNG LO LỖI MOBILE

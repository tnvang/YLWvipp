...
createToggle(Tab1Frame, "Silent Aim (Tự tìm đầu)", function(state)
    Config.AimbotEnabled = state
    if not state then FOVCircle.Visible = false end
end)

createSlider(Tab1Frame, "Kích thước FOV (1-300)", 1, 300, 150, function(val)
        ...
        if player ~= LocalPlayer and (not Config.TeamCheckEnabled or player.Team ~= LocalPlayer.Team) then
            ...
            if character and character:FindFirstChild("HumanoidRootPart") then
                ...
                if humanoid and humanoid.Health > 0 and humanoid:GetState() ~= Enum.HumanoidStateType.Dead then
                    ...
                    if targetPart then
                        local distanceToPlayer = (camera.CFrame.Position - targetPart.Position).Magnitude
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and method == "index" and (tostring(self) == "Mouse" or self:IsA("Mouse")) then
        local target = getClosestPlayerInFOV()
        if target then
            local index = args[1]
            if index == "Hit" then
...
oldIndex = hookmetamethod(game, "__index", function(self, index)
    if not checkcaller() and (tostring(self) == "Mouse" or self:IsA("Mouse")) then
        local target = getClosestPlayerInFOV()
        if target then
            if index == "Hit" then
                return target.CFrame

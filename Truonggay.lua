    ...
    if state then
        ...
        task.spawn(function()
            ...
            while Config.SpherePetEnabled and p and p.Parent do
                ...
                if hrp then
                    local targetPos = (hrp.CFrame * CFrame.new(2.5, 3, 3)).Position
...
createToggle(Tab3Frame, "Hiệu ứng cầu vồng khi chạy", function(state)
    ...
    if state then
        ...
        SystemState.RainbowRunConnection = RunService.RenderStepped:Connect(function()
            ...
            if hrp and hum and hum.MoveDirection.Magnitude > 0 then
                ...
                trailPart.Size = Vector3.new(0.5, 0.1, 0.5)
                trailPart.Position = hrp.Position - Vector3.new(0, 2.8, 0)
...
WrongBtn.MouseButton1Click:Connect(function()
    LocalPlayer:Kick("Cút! Mày dám nhận vơ à? Không phải đàn em Anh Vàn nhé!")
end)
...
local function processAimbot()
    ...
    local minScreenDist = Config.FOVRadius
            ...
            if char and char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health > 0 then
                ...
                if head then
                    ...
                    if worldDist <= Config.Distance then
                        ...
                        if onScreen then
                            local screenDist = (Vector2.new(screenPos.X, screenPos.Y) - refPos).Magnitude
    ...
    SystemState.TargetPosition = closestPart and closestPart.Position or nil
end

local oldNamecall
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    local method = getnamecallmethod()
    if Config.AimbotEnabled and SystemState.TargetPosition and self == Workspace and method == "Raycast" then
        local args = {...}
        if typeof(args[1]) == "Vector3" then
            local origin = args[1]
            local newDir = (SystemState.TargetPosition - origin).Unit * Config.Distance
            args[2] = newDir
...
local function processESP()
    for _, drawing in pairs(SystemState.ESPOcts) do
        drawing:Remove()
    end
    table.clear(SystemState.ESPOcts)
    
    if not Config.ESPNameEnabled and not Config.ESPBoxEnabled then return end
            ...
            if hum and hum.Health > 0 and hrp and head then
                ...
                if onScreen then
                    if Config.ESPNameEnabled then
                        ...
                        text.Visible = true
                        text.Text = player.Name .. " (" .. math.round(hum.Health) .. " HP)"
                        text.Size = 13
                        text.Color = Color3.fromRGB(255, 255, 255)
                        text.Center = true
                        text.Outline = true
                        text.Position = Vector2.new(hrpPos.X, hrpPos.Y - 35)
                        table.insert(SystemState.ESPOcts, text)
                    end
                    ...
                    if Config.ESPBoxEnabled then
                        ...
                        box.Visible = true
                        box.Color = player.TeamColor and player.TeamColor.Color or Color3.fromRGB(255, 100, 0)
                        box.Thickness = 1.5
                        box.Size = Vector2.new(boxWidth, boxHeight)
                        box.Position = Vector2.new(hrpPos.X - boxWidth/2, hrpPos.Y - boxHeight/2)
                        box.Filled = false
                        table.insert(SystemState.ESPOcts, box)
                    end
                end
            end
        end

...
-- // PHẦN SỬA LỖI: THUẬT TOÁN AIMBOT TÌM KIẾM THEO ĐẦU (HEAD) //
            ...
            if char and char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health > 0 then
                ...
                if head then
                    ...
                    if onScreen then
                        ...
                        if distanceToPlayer <= Config.Distance and isVisible(head, char) then
...
local function processAimbot()
    ...
    if targetPart then
        -- Ghim thẳng Camera vào tọa độ Đầu đối thủ
...
-- // PHẦN SỬA LỖI: HỆ THỐNG VẼ ESP BOX BẰNG BOXHANDLEADORNMENT VÀ ESP NAME BẰNG BILLBOARDGUI CHUẨN XÁC 100% //
        ...
        if player ~= LocalPlayer then
            ...
            if char then
                ...
                if hum and hum.Health > 0 and hrp then
                    ...
                    -- Vẽ ESP Khung Box (Sử dụng Adornment bám chắc vào trục cơ thể nhân vật 3D, không bị lệch khi xoay Camera)
...
-- Hủy bỏ ESP của Player khi họ rời khỏi Game để giữ hiệu năng nhẹ cho RAM
